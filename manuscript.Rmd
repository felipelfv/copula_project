...
for (cfg in configs) {
  ...
  for (i in seq_along(cfg$pars)) {
    ...
    label <- sprintf("%s%s(%s,%s,%s)", display_fam, rot_label, par_xz_str, par_yz_str, par_xy_z_str)
...
Within each scenario, we consider multiple copula families (Frank, Gumbel, Clayton, and Gaussian) across a range of parameter values to ensure robustness of the findings. Negative dependence for Gumbel and Clayton copulas is achieved via 90-degree rotation.
...
ggplot(plot_data_11c, aes(x = U_X, y = U_Y)) +
  ...
  scale_color_distiller(palette = "Blues", direction = 1, name = "Z") +
...
Finally, a simulation study was conducted to compare how the partial and the marginal copulas may be different. In scenarios presenting conditional independence (scenarios 2, 3 and 4), the partial copula is the independence copula, while the marginal copula contains the spurious association induced by $Z$. In scenarios where $Z$ is both positively associated to $X$ and $Y$ or both negatively associated to them (1,5,6,7), the association observed in the marginal copula was stronger, while in the scenarios with "opposite confounding" (8 and 9) the partial copula shows stronger dependence. In scenarios with no confounding (10), the marginal and the partial copula are the same. Finally, scenario 11 illustrated cases violating simplifying assumption, which shows a fundamental difference between partial and conditional copulas: conditional copulas depend on the specification of the value being conditioned in the variable $Z$, while the partial copula does not. In this sense, the partial copula may be seen as a "summary" of the conditional copulas across all values of $Z$.
...
```{r scatter-1, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 1: Positive confounding, positive conditional dependence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$). Each panel displays $\\rho$ (Spearman) and $\\tau$ (Kendall). Color gradient represents $Z$."}
create_scenario_scatter(all_data, 1)
```

```{r scatter-2, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 2: Positive confounding, conditional independence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$), where $\\text{ind}$ denotes independence."}
create_scenario_scatter(all_data, 2)
```

```{r scatter-3, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 3: Negative confounding, conditional independence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$), where $\\text{ind}$ denotes independence. (R) indicates 90-degree rotation to model negative dependence."}
create_scenario_scatter(all_data, 3)
```

```{r scatter-4, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 4: Opposite-sign confounding, conditional independence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$), where $\\text{ind}$ denotes independence."}
create_scenario_scatter(all_data, 4)
```

```{r scatter-5, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 5: Positive confounding, negative conditional dependence (Simpson's paradox). Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$). Cases marked with $*$ use stronger confounding ($\\theta_{XZ}, \\theta_{YZ}$) combined with weaker conditional dependence ($\\theta_{XY|Z}$), illustrating how strong confounding can almost entirely mask a weak opposing conditional effect."}
create_scenario_scatter(all_data, 5)
```

```{r scatter-6, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 6: Negative confounding, positive conditional dependence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$). (R) indicates 90-degree rotation."}
create_scenario_scatter(all_data, 6)
```

```{r scatter-7, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 7: Negative confounding, negative conditional dependence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$). (R) indicates 90-degree rotation."}
create_scenario_scatter(all_data, 7)
```

```{r scatter-8, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 8: Opposite-sign confounding, positive conditional dependence (Simpson's paradox). Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$)."}
create_scenario_scatter(all_data, 8)
```

```{r scatter-9, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 9: Opposite-sign confounding, negative conditional dependence. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$)."}
create_scenario_scatter(all_data, 9)
```

```{r scatter-10, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 10: No confounding. Row labels show Family($\\theta_{XZ}, \\theta_{YZ}, \\theta_{XY|Z}$), where $\\text{ind}$ denotes independence."}
create_scenario_scatter(all_data, 10)
```

```{r scatter-11, echo=FALSE, dev="png", dpi=300, out.width="100%", fig.cap="Scenario 11: Simplifying assumption violated. The conditional copula parameter $\\theta(z)$ varies with $Z$."}
