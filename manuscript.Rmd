...
\begin{remark}
Let $g(x, y, z) = C_{X,Y|Z=z}(x, y)$. $g(x, y, Z)$ is a random variable, and Proposition \ref{def:partial-copula} entails that
...
The remainder of the proof comes from remembering that the partial copula evaluated in a point $(x, y) \in [0, 1]^2$ is the expectation of a random variable (see Remark \ref{rem:partial-copula-necessary-not-sufficient}), which enables using the monotonicity of expectation: property E2 of section 3.3 of \citet{james1981} readily entails that, if a random variable belongs almost surely to an interval, then the same happens to its expectation. Therefore,
...
\begin{proposition}\label{def:pearson-bounds}
...
\begin{proof}
The proof of Proposition \ref{def:pearson-bounds} entails that, if $(D(X, Y | Z = z)) \leq k$ for all $z \in \mathbb{R}$, then
$$xy - \frac{k}{4} \leq C_{X,Y;Z}(x, y) \leq xy + \frac{k}{4},$$
which entails that
$$\int_0^1 \int_0^1 C_{X,Y;Z}(x, y) dx dy \in \left[\frac{1-k}{4}, \frac{1+k}{4}\right].$$

Theorem 5.1.6 in \citet{nelsen2006} states that, given a copula $C$, its Pearson's correlation can be represented as $12 \int_0^1 \int_0^1 C_{X,Y;Z}(x, y) dx dy - 3$. Applying this representation in the partial copula, it holds that
$$-3k \leq \rho(X, Y; Z) \leq 3k,$$
as we wished to prove. The final bound in the proposition is obtained taking into account that Pearson's coefficient belongs to $[-1, 1]$ range \citep[see Theorem 5.1.9 in][]{nelsen2006}.
\end{proof}
...
\begin{proof}
We have seen in the proof of Proposition \ref{def:pearson-bounds} that, if $(D(X, Y | Z = z)) \leq k$ for all $z \in \mathbb{R}$, then
$$xy - \frac{k}{4} \leq C_{X,Y;Z}(x, y) \leq xy + \frac{k}{4}.$$
...
As in the previous osu proof, the final bounds are obtained using the fact that Kendall's coefficient belongs to the $[-1, 1]$ range \citep[see Theorem 5.1.9 in][]{nelsen2006}.
\end{proof}
...
\begin{proof}
This result is a direct consequence of applying Proposition \ref{prop:partial-copula} in the conditional copulas $C_{X,Y|Z=z}(x, y) = xy + h(z)f(x)g(y)$.
\end{proof}
...
We consider eleven scenarios, summarized in Table 1. The triplet $(s_{XZ}, s_{YZ}, s_{XY|Z})$ indicates the sign of dependence in each pair copula: $+$ for positive, $-$ for negative, and $\varnothing$ for independence. Scenarios 1--10 satisfy the simplifying assumption, so that $C_{X,Y|Z=z}$ does not vary with $z$ and Corollary \ref{cor:partial-conditional-copula} applies. Scenario 11 violates the simplifying assumption by letting the conditional copula parameter vary as a function of $Z$ through specifications $\theta(z) = \exp(z)$, $\theta(z) = -\exp(z)$, and $\theta(z) = 1 - 2z$; the last of these illustrates Remark \ref{rem:remark-2}, where positive and negative conditional associations cancel out in expectation.
...
| 2 | $(+,+,\varnothing)$ | Positive confounding, conditional indep. |
| 3 | $(-,-,\varnothing)$ | Negative confounding, conditional indep. |
| 4 | $(+,-,\varnothing)$ | Opposite confounding, conditional indep. |
