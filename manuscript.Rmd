...
An even more general result is available in @rosenblatt1952: @rosenblatt1952 proved that, given a sequence of random variables $X_1, \ldots, X_n$, the random variables
...
The remainder of the proof comes from remembering that the partial copula evaluated in a point $(x, y) \in [0, 1]^2$ is the expectation of a random variable (see Remark 3.3), which enables using the monotonicity of expectation: property E2 of section 3.3 of \citet{james1981} readily entails that, if a random variable belongs almost surely to an interval, then the same happens to its expectation. Therefore,
...
\begin{proof}
The proof of Proposition 5 entails that, if $(D(X, Y | Z = z)) \leq k$ for all $z \in \mathbb{R}$, then
$$xy - \frac{k}{4} \leq C_{X,Y;Z}(x, y) \leq xy + \frac{k}{4},$$
which entails that
...
\begin{proof}
We have saw in the proof of Proposition 5 that, if $(D(X, Y | Z = z)) \leq k$ for all $z \in \mathbb{R}$, then
$$xy - \frac{k}{4} \leq C_{X,Y;Z}(x, y) \leq xy + \frac{k}{4}.$$
...
\begin{proof}
This result is a direct consequence of applying Proposition 3.1 in the conditional copulas $C_{X,Y|Z=z}(x, y) = xy + h(z)f(x)g(y)$.
\end{proof}
...
We consider eleven scenarios, summarized in Table 1. The triplet $(s_{XZ}, s_{YZ}, s_{XY|Z})$ indicates the sign of dependence in each pair copula: $+$ for positive, $-$ for negative, and $\varnothing$ for independence. Scenarios 1--10 satisfy the simplifying assumption, so that $C_{X,Y|Z=z}$ does not vary with $z$ and Corollary 3.1 applies. Scenario 11 violates the simplifying assumption by letting the conditional copula parameter vary as a function of $Z$ through specifications $\theta(z) = \exp(z)$, $\theta(z) = -\exp(z)$, and $\theta(z) = 1 - 2z$; the last of these illustrates Remark 3.2, where positive and negative conditional associations cancel out in expectation.
